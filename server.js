...
// Enhanced Subscription Plans with Categories
...
app.post('/api/donate', async (req, res) => {
  try {
    ...
    if (donationAmount < 1) {
      return res.status(400).json({ success: false, error: 'Minimum donation amount is KES 1' });
    }
    if (donationAmount > 150000) {
      return res.status(400).json({ success: false, error: 'Maximum donation amount is KES 150,000' });
    }
    ...
    res.json({
      ...
      data: {
        ...
        thankYouMessage:
          'Thank you for supporting Bera Tech! Your contribution helps us improve our services.',
        isDonation: true,
      },
    });
  } catch (error) {
